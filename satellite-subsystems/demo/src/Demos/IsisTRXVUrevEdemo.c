...
static Boolean softResetVUTest(void)
{
	...
	print_error(isis_vu_e__reset_wdg_rx(0));
	vTaskDelay(1 / portTICK_RATE_MS);
	print_error(isis_vu_e__reset_wdg_tx(0));
...
static Boolean hardResetVUTest(void)
{
	...
	print_error(isis_vu_e__reset_hw_rx(0));
	vTaskDelay(1 / portTICK_RATE_MS);
	print_error(isis_vu_e__reset_hw_tx(0));
...
static Boolean vutc_sendDefClSignTest(void)
{
	...
	while(txCounter < 5 && timeoutCounter < 5)
	{
		...
		print_error(isis_vu_e__send_frame(0, testBuffer1, 10, &avalFrames));
...
static Boolean vutc_toggleIdleStateTest(void)
{
	...
	if(toggle_flag)
	{
	    print_error(isis_vu_e__set_idle_state(0, isis_vu_e__onoff__off));
		toggle_flag = FALSE;
	}
	else
	{
	    print_error(isis_vu_e__set_idle_state(0, isis_vu_e__onoff__on));
		toggle_flag = TRUE;
	}
...
{
    print_error(isis_vu_e__set_bitrate(0, isis_vu_e__bitrate__9600bps));
...
{
    print_error(isis_vu_e__set_bitrate(0, isis_vu_e__bitrate__1200bps));
...
static Boolean vurc_getFrameCountTest(void)
{
	...
	{
	    print_error(isis_vu_e__get_frame_count(0, &RxCounter));
...
static Boolean vurc_getFrameCmdTest(void)
{
	...
	isis_vu_e__get_frame__from_t rxFrameCmd = {0,0,0, rxframebuffer};
	...
	while(RxCounter > 0)
	{
		print_error(isis_vu_e__get_frame(0, &rxFrameCmd));
		...
		vTaskDelay(10 / portTICK_RATE_MS);
		print_error(isis_vu_e__remove_frame(0));
		vTaskDelay(10 / portTICK_RATE_MS);
...
static Boolean vurc_getFrameCmdAndTxTest(void)
{
	...
	while(RxCounter > 0)
	{
		...
		printf("Frequency offset (Doppler) for received frame: %.2f Hz\r\n", ((double)rxFrameCmd.doppler) * -38.1469726563 + 0); // Only valid for rev. B & C boards
		printf("Received signal strength (RSSI) for received frame: %.2f dBm\r\n", ((double)rxFrameCmd.rssi) * -0.5 + -22);
		...
		isis_vu_e__send_frame(0, rxframebuffer, SIZE_RXFRAME, &avalFrames);
...
static void vurc_revEInterruptCallback()
{
...
static Boolean vurc_getFrameCmdInterruptTest(void)
{
    ...
    TRXVU_FR_Enable(vurc_revEInterruptCallback);
...
static Boolean vurc_getRxTelemTest_revE(void)
{
	...
	eng_value = ((float)telemetryValue) * (-0.5) - 22;
	...
	eng_value = ((float)telemetryValue) * 0.3052;
...
static Boolean vutc_getTxTelemTest_revE(void)
{
	unsigned short telemetryValue;
	float eng_value = 0.0;
	uint32_t uptime = 0;
	isis_vu_e__get_tx_telemetry__from_t telemetry;
	int rv;

	// Telemetry values are presented as raw values
	printf("\r\nGet all Telemetry at once in raw values \r\n\r\n");
	rv = isis_vu_e__get_tx_telemetry(0, &telemetry);
	if(rv)
	{
		printf("Subsystem call failed. rv = %d", rv);
		return TRUE;
	...
	rv = isis_vu_e__tx_uptime(0, &uptime);
	if(rv)
	{
		printf("Subsystem call failed. rv = %d", rv);
		return TRUE;
...
static void _WatchDogKickTask(void *parameters)
{
    ...
    for(;;)
    {
        (void)isis_vu_e__reset_wdg_rx(0);
        (void)isis_vu_e__reset_wdg_tx(0);
...
static Boolean selectAndExecuteTRXVUDemoTest(void)
{
	...
	printf("\t 13) Get RX/TX frequencies \n\r");
	printf("\t 14) Start a watchdog kick task. This cannot be stopped without reboot. \n");
    ...
    case 10:
        ...
        break;
	case 11:
		offerMoreTests = vurc_getRxTelemTest_revE();
		break;
	case 12:
		offerMoreTests = vutc_getTxTelemTest_revE();
		break;
	case 13:
		offerMoreTests = vu_getFrequenciesTest_revE();
		break;
	case 14:
...
{
	if(IsisTRXVUrevEdemoInit())									// initialize of I2C and IsisTRXVU subsystem drivers succeeded?
	{
		IsisTRXVUrevEdemoLoop();								// show the main IsisTRXVU demo interface and wait for user input
		return TRUE;
	}
	else
	{
